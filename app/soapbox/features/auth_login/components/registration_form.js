...
const messages = defineMessages({
  ...
  pp: { id: 'registration.pp', defaultMessage: 'Privacy Policy' },
...
class RegistrationForm extends ImmutablePureComponent {
  ...
  render() {
    const { instance, intl, inviteToken, supportsEmailList } = this.props;
      ...
      <SimpleForm onSubmit={this.onSubmit}>
        ...
        <fieldset disabled={isLoading}>
          <div className='simple_form__overlay-area'>
            ...
            <div className='fields-group'>
              <Checkbox
                label={intl.formatMessage(messages.agreement, { tos: <Link to='/legal/terms-of-service' target='_blank' key={0}>{intl.formatMessage(messages.tos)}</Link>, pp: <Link to='/legal/privacy-policy' target='_blank' key={1}>{intl.formatMessage(messages.pp)}</Link> })}
                name='agreement'
